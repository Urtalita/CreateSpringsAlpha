...
public class CSStress extends ConfigBase {
    ...
    public static <B extends Block, P> NonNullUnaryOperator<BlockBuilder<B, P>> setImpact(double value) {
        return builder -> {
            ...
            ResourceLocation id = CreateSprings.asResource(builder.getName());
    ...
    private static void assertFromCreateCasing(BlockBuilder<?, ?> builder) {
        if (!builder.getOwner().getModid().equals(CreateSprings.MODID)) {
            throw new IllegalStateException("Non-Create Springs blocks cannot be added to Create Spring's config.");
        }
    }
