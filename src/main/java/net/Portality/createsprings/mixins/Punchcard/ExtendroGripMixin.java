...
public class ExtendroGripMixin extends ItemDummyMixin{
    ...
    public void onInventoryTick(ItemStack pStack, Level pLevel, Entity pEntity, int pSlotId, boolean pIsSelected, CallbackInfo ci) {
        ...
        if(pEntity instanceof Player player){
            if(pLevel.getGameTime() % 10 == 0){
                if(pIsSelected){
                    PunchcardInterpritator.DoPunchcardLogic(new ExecutorInfo(pStack, pLevel, player, pSlotId, pSlotId, PunchcardExecutor.EXTENDRO_GRIP, AllItems.EXTENDO_GRIP.get()));
                } else {
                    PunchcardInterpritator.DoPunchcardLogic(new ExecutorInfo(pStack, pLevel, player, pSlotId, pSlotId + 1, PunchcardExecutor.EXTENDRO_GRIP, AllItems.EXTENDO_GRIP.get()));
                }
            }
        }
    }
