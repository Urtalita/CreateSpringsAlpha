...
public class BackTankMixin extends ItemDummyMixin{
    ...
    public void onInventoryTick(ItemStack pStack, Level pLevel, Entity pEntity, int pSlotId, boolean pIsSelected, CallbackInfo ci) {
        ...
        if(pEntity instanceof Player player){
            if(pLevel.getGameTime() % 10 == 0){
                if(pIsSelected){
                    PunchcardInterpritator.DoPunchcardLogic(new ExecutorInfo(pStack, pLevel, player, pSlotId, true, PunchcardExecutor.BACKTANK, AllItems.COPPER_BACKTANK.get()));
                } else {
                    PunchcardInterpritator.DoPunchcardLogic(new ExecutorInfo(pStack, pLevel, player, pSlotId, false, PunchcardExecutor.BACKTANK, AllItems.COPPER_BACKTANK.get()));
                }
            }
        }
    }
    ...
    public void overrideOtherStackedOnMe(ItemStack pStack, ItemStack pOther, Slot pSlot, ClickAction pAction, Player pPlayer, SlotAccess pAccess, CallbackInfoReturnable<Boolean> cir) {
        ...
        SpringPoweredCore core = new SpringPoweredCore(0, allowedModifficators);
        if (core.overrideOtherStackedOnMe(pStack, pOther, pSlot, pAction, pPlayer, pAccess)) {
